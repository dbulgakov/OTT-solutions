...
public class OrdersPresenterTest extends BaseTest{
    ...
    public void setUp() throws Exception {
        ...
        // кажется, здесь подойдет такая конструкция
        when(model.getUserOrders(TestConst.TEST_USER_ID_WITH_ORDERS)).thenReturn(Observable.just(orderList));
    }
    ...
    private List<UserOrder> filterOrders(List<UserOrder> userOrders, Predicate<UserOrder> userOrderPredicate){
        // дублируем часть кода, который есть в презентере.. В идеале стоит избегать таких вещей
        // в данном случае можно попробовать вынести в отдельный метод в презентере часть, которая делает
        // flatMap, filter, toSortedList и ее использовать
